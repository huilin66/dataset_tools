...
# from configs import DISPLAY_LEVELS, LEVEL_MAP
...
class PDFExporterCompact(BasePDFExporter):
    ...
    def _add_summary_pages(self, elements, report_data):
        ...
        def add_level_table(title, row_keys, stats_dict, col1_name):
            ...
            header = [col1_name] + DISPLAY_LEVELS + ["Total"]
            data = [header]
            ...
            col_w = [2.0*inch] + [1.0*inch]*len(DISPLAY_LEVELS) + [0.8*inch]
    ...
    #     # --- 数据聚合 (包含修复后的 View 提取逻辑) ---
    #     stats_cat_lev = defaultdict(lambda: defaultdict(int))   # [Defect Type][Severity]
    #     stats_dir_lev = defaultdict(lambda: defaultdict(int))   # [Direction][Severity]
    #     stats_floor_lev = defaultdict(lambda: defaultdict(int)) # [Floor][Severity]
        
    #     # A. 获取预定义类别
    #     defined_cats = report_data.get('defined_categories', [])
    #     all_categories = set(defined_cats)
        
    #     # B. 【新增】获取预定义楼层 (来自 floor_map.json)
    #     defined_floors = report_data.get('defined_floors', [])
    #     all_floors = set(defined_floors)  # 初始化所有楼层，确保 0 数据也能显示
    ...
    #         # 获取 Direction (View + Orientation)
    #         v = df.iloc[0].get('view', 'Unknown').strip()
    #         o = df.iloc[0].get('orientation', '').strip()
    #         direction_label = f"{v}\n{o}" if o else v
    #         all_directions.add(direction_label)
    ...
    #             # 获取 Floor (直接使用数据中自带的字段，不重新计算)
    #             fl = str(row.get('floor', 'Unknown'))
                
    #             # 如果数据里出现了 floor_map 以外的楼层，也加进去
    #             all_floors.add(fl)
    #             all_categories.add(cat)

    #             # 累加计数
    #             stats_cat_lev[level][cat] += 1
    #             stats_dir_lev[level][direction_label] += 1
    ...
    #     # 方向排序
    #     sorted_dirs = sorted(list(all_directions))
        
    #     floor_order = {floor: index for index, floor in enumerate(defined_floors)}
    #     # 楼层排序 (使用智能排序，reverse=True 表示从顶层 ROOF 到底层 LG)
    #     sorted_floors = sorted(list(all_floors), key=lambda floor: floor_order.get(floor, float('inf')))

    #     # === 4. 通用表格生成函数 ===
    #     def add_stat_table(title, row_keys, stats_dict, col1_name):
    #         elements.append(Paragraph(title, self.styles["font_section"]))
    #         header = [col1_name] + DISPLAY_LEVELS + ["Total"]
    #         data = [header]
    ...
    #             for lvl in DISPLAY_LEVELS:
    #                 val = stats_dict[lvl][key]
    #                 row.append(str(val) if val > 0 else "0") # 0 也会显示
    #                 total += val
    #             row.append(str(total))
    #             data.append(row)
    ...
    #     # === 5. 输出三个表格 ===
    #     # A. Defect Type
    #     add_stat_table("Summary by Defect Type & Severity:", sorted_cats, stats_cat_lev, "Defect Type")

    #     # B. Direction (新增)
    #     add_stat_table("Summary by Direction & Severity:", sorted_dirs, stats_dir_lev, "Direction")

    #     # C. Floor (新增 - 基于 floor_map.json 的完整名单)
    #     add_stat_table("Summary by Floor & Severity:", sorted_floors, stats_floor_lev, "Floor")
