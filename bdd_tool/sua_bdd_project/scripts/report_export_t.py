...
def main():
    ...
    engine = BatchDedupEngine(
        ...
        explanation_json=config.EXPLANATION_T_JSON,
        floor_map_path=config.HEIGHT_MAP_JSON,
    )
    os.makedirs(config.REPORT_DIR_T, exist_ok=True)
    ...
    for view_name in views_list:
        ...
        if config.VIEW_REPORT:
            engine.export_aggregated_report(
                all_df=df, 
                output_path=config.REPORT_VIEW_T_PATH.replace('.pdf', f'_{view_name}.pdf'),
                style_id=config.REPORT_VIEW_STYLE_ID_T,
                logo_left=config.LOGO1,
                logo_right=config.LOGO2,
                target_cls_names=config.TARGET_CLASSES_NAME_T,
                max_workers=config.NUM_WORKERS,
            )
    ...
    if all_dfs:
        ...
        engine.export_aggregated_report(
            all_df=final_df, 
            output_path=config.REPORT_OVERALL_T_PATH,
            style_id=config.REPORT_OVERALL_STYLE_ID_T,
            logo_left=config.LOGO1,
            logo_right=config.LOGO2,
            target_cls_names=config.TARGET_CLASSES_NAME_T,
            max_workers=config.NUM_WORKERS,
