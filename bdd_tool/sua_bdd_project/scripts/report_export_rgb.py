...
def main():
    ...
    engine = BatchDedupEngine(
        ...
        explanation_json=config.EXPLANATION_RGB_JSON,
        floor_map_path=config.HEIGHT_MAP_JSON,
    )
    os.makedirs(config.REPORT_DIR_RGB, exist_ok=True)
    ...
    for view_name in views_list:
        ...
        df = engine.process_view_data_aux(
            ...
            class_path=config.CLASS_RGB_TXT,
        ...
        if config.VIEW_REPORT:
            engine.export_aggregated_report(
                all_df=df, 
                output_path=config.REPORT_VIEW_RGB_PATH.replace('.pdf', f'_{view_name}.pdf'),
                style_id=config.REPORT_VIEW_STYLE_ID_RGB,
                logo_left=config.LOGO1,
                logo_right=config.LOGO2,
                target_cls_names=config.TARGET_CLASSES_NAME_RGB,
                max_workers=config.NUM_WORKERS,
            )
    ...
    if all_dfs:
        ...
        engine.export_aggregated_report(
            all_df=final_df, 
            output_path=config.REPORT_OVERALL_RGB_PATH,
            style_id=config.REPORT_OVERALL_STYLE_ID_RGB,
            logo_left=config.LOGO1,
            logo_right=config.LOGO2,
            target_cls_names=config.TARGET_CLASSES_NAME_RGB,
            max_workers=config.NUM_WORKERS,
