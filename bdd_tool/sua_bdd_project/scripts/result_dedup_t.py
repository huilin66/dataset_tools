...
def main():
    ...
    target_classes = [class_names.index(cls) for cls in config.TARGET_CLASSES_NAME_T] if config.TARGET_CLASSES_NAME_T is not None else list(range(len(class_names)))
    ...
    for view_name in views_list:
        ...
        all_dets = yolo_projecting(image_dir, yolo_dir, exif_db, floor_manager, conf_thresh=config.CONF_THRESH_PRIMARY, target_classes=target_classes, start_gid=view_id_offset)
        if all_dets is None:
            continue
        print(">>> Done!\n")
