...
class VideoRecorder(QMainWindow):
    ...
    def start_recording(self):
        ...
        self.recording = True
        self.recording_thread_rgb = threading.Thread(target=self.record_video_streams_rgb)
        self.recording_thread_rgb.start()
        self.recording_thread_thermal = threading.Thread(target=self.record_video_streams_thermal)
        self.recording_thread_thermal.start()
    ...
    def stop_recording(self):
        ...
        print(datetime.datetime.now())
        self.recording = False
        if self.recording_thread_rgb:
            self.recording_thread_rgb.join()
        if self.recording_thread_thermal:
            self.recording_thread_thermal.join()
        self.start_button.setEnabled(True)
        self.stop_button.setEnabled(False)

    def record_video_streams_rgb(self):
        ...
        while self.recording:
            ...
            print(f"Recording rgb ... Elapsed time: {current_time - start_time} seconds")

        end_time = time.time()
        print(f"Recording rgb duration: {end_time - start_time} seconds")

        # Release resources when done
        self.rgb_cap.release()
        self.rgb_out.release()

    def record_video_streams_thermal(self):
        start_time = time.time()
        while self.recording:
            ...
            # Log the current timestamp to diagnose timing issues
            current_time = time.time()
            print(f"Recording thermal ... Elapsed time: {current_time - start_time} seconds")

        end_time = time.time()
        print(f"Recording thermal duration: {end_time - start_time} seconds")
