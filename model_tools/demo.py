...
if __name__ == '__main__':
    pass
    model = MLDecoder(10, num_of_groups=10, decoder_embedding=1024).to('cuda')
    data = [torch.randn(4, 320, 32, 32).to('cuda'),
            torch.randn(4, 640, 16, 16).to('cuda'),
            torch.randn(4, 640, 8, 8).to('cuda')]
    for d in data:
        result = model(d)
        print(result.shape)
