...
def dir_vis(json_dir, input_dir1, input_dir2, input_dir3, input_dir4, input_dir5, input_dir6, vis_item='abcd', small_threshold=None):
    ...
    for json_name in tqdm(json_list):
        ...
        if os.path.exists(input1_path) and os.path.exists(input2_path) and os.path.exists(input3_path) and os.path.exists(input4_path) and os.path.exists(input5_path) and os.path.exists(input6_path) and os.path.exists(json_path):   
            files_vis(json_path, input1_path, input2_path, input3_path, input4_path, input5_path, input6_path, vis_item=vis_item, small_threshold=small_threshold)
        else:
            print(f'{json_name} sample missing')


def files_vis(json_path, input1_path, input2_path, input3_path, input4_path, input5_path, input6_path, vis_item='abcd', small_threshold=None):
    ...
    img1, with_defect = img_vis(img1, data1, vis_item=vis_item, small_threshold=small_threshold)
    if with_defect:
    ...
    img2, with_defect = img_vis(img2, data2, vis_item=vis_item, small_threshold=small_threshold)
    if with_defect:
    ...
    img3, with_defect = img_vis(img3, data3, vis_item=vis_item, small_threshold=small_threshold)
    if with_defect:
    ...
    img4, with_defect = img_vis(img4, data4, vis_item=vis_item, small_threshold=small_threshold)
    if with_defect:
    ...
    img5, with_defect = img_vis(img5, data5, vis_item=vis_item, small_threshold=small_threshold)
    if with_defect:
    ...
    img6, with_defect = img_vis(img6, data6, vis_item=vis_item, small_threshold=small_threshold)
    if with_defect:
...
def img_vis(img, results, vis_item='abcd', small_threshold=None):
    ...
    for result in results:
        ...
        if small_threshold is not None and (box_width<small_threshold and box_height<small_threshold):
            continue
        ...
        cv2.putText(img, defect_str, (int(top_left_x), int(top_left_y) + 12), cv2.FONT_HERSHEY_SIMPLEX, 0.5, [0, 0, 0], 1)
    if risk_sum == 0:
        with_defect = False
    else:
        with_defect = True
...
def json2yolo(json_file):
    ...
    for i in range(6):
        ...
        for record in data:
            ...
            nums = [cat_id, 4, risk_a_value, risk_b_value, risk_c_value, risk_d_value] + uvs
...
def select_defect(input_dir, output_dir, ref_dir, src_image_dirname='images_select'):
    ...
    for label_name in tqdm(label_list):
        ...
        input_image_path = os.path.join(input_image_dir, label_name.replace('.txt', '.jpg'))
        ...
        output_image_path = os.path.join(output_image_dir, label_name.replace('.txt', '.jpg'))
        result = check_defect(input_label_path)
        if result:
            shutil.copy(input_label_path, output_label_path)
            shutil.copy(input_image_path, output_image_path)
    print(f'select {len(os.listdir(output_image_dir))} from {len(os.listdir(input_image_dir))}')
    ...
    with open(output_file, 'w') as f_out:
        for idx, img_path in enumerate(tqdm(img_files), 1):
            ...
            for line in lines:
                ...
                class_id, x_center, y_center, width, height, group_id = map(float, parts)
...
def rename_yolo_files(input_dir, output_dir, prefix_map=None):
    ...
    for image_file in tqdm(image_files):
        ...
        new_base_name = f'{prefix_number:01d}_{timestamp}' if timestamp else f'{prefix_number:01d}'
...
def select_topk_timestamp_files(input_dir, output_dir, k=5):
    ...
    print(f'选择完成: 共处理{total_files}个文件，来自{len(topk_timestamps)}个时间戳，已保存到{output_dir}')
...
def rename_yolo_files(input_dir, output_dir, prefix_map=None):
    ...
    for image_file in tqdm(image_files):
        ...
        # 新文件名格式: 数字_timestamp.ext
        new_base_name = f'{prefix_number:01d}{timestamp}' if timestamp else f'{prefix_number:01d}'
        new_image_name = new_base_name + ext
        new_label_name = new_base_name + '.txt'
