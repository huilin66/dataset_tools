...
def yolo2coco_arg(arg):
    ...
    # originLabelsDir = root_path
    # originImagesDir = root_path
    with open(os.path.join(root_path,'classes.txt')) as f:
        classes = f.read().strip().split()
    # images dir name
    # indexes = os.listdir(originImagesDir)

    indexes = pd.read_csv(arg.list_path, header=None)[0].to_list()
    ...
    for k, index in enumerate(tqdm(indexes)):
        ...
        # 支持 png jpg 格式的图片。
        txtFile = index.replace('.png', '.txt').replace('.JPG', '.txt')
        # 读取图像的宽和高
        ...
        # 添加图像的信息
        dataset['images'].append({'file_name': os.path.join(os.path.basename(os.path.dirname(index)), os.path.basename(index)),
                                  'id': k,
                                  'width': width,
                                  'height': height})
...
def yolo2coco():
    ...
    indexes = pd.read_csv(list_path, header=None)[0].to_list()
...
def get_arg():
    ...
    parser.add_argument('--list_path', type=str,
                        default=r'E:\Huilin\download\YOLO\data\SARAOS\SARAOS-AOS-noAHE_%s.txt'%split_set,
                        help="if not split the dataset, give a path to a json file")
    parser.add_argument('--root_dir', default=r'E:\Huilin\download\YOLO', type=str,
                        help="root path of images and labels, include ./images and ./labels and classes.txt")
    parser.add_argument('--save_path', type=str, default=r'E:\Huilin\download\YOLO\data\SARAOS\aos\%s\%s.json'%(split_set.upper(), split_set),
                        help="if not split the dataset, give a path to a json file")
